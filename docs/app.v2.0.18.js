...
/* ===== V2.0.18 - Nombre oficial del sistema ===== */
(function(){
  const APP_VERSION_V2 = "2.0.18";
...
/* ===== V2.0.18 - Version y menu lateral robustos ===== */
(function(){
...
/* ===== V2.0.18 - Anti-cache y normalizacion final menu/version ===== */
(function(){
  const VERSION='2.0.18';
  window.ELTA_APP_VERSION = VERSION;
...
/* ===== V2.0.18 - Correcciones GPS, clima, alertas y seguimiento ===== */
(function(){
  const VERSION='2.0.18';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.18 - Ajustes finales clima, KM y alertas ===== */
(function(){
  const VERSION='2.0.18';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.18 - Ajuste final: menu, clima compacto, KM y alertas alineadas ===== */
(function(){
  const VERSION='2.0.18';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.18 - Correccion funcional final: menu, clima, KM y alertas compactas ===== */
(function(){
  const VERSION='2.0.18';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.18 - Vista Alertas compacta: boton Verificar visible ===== */
(function(){
  const VERSION='2.0.18';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.18 - Torre de Control: Últimas alertas formato linea unica ===== */
(function(){
  const VERSION='2.0.18';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.18 - Seguimiento: datos a la izquierda y alertas a la derecha ===== */
(function(){
  const VERSION='2.0.18';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
  ...
  const open=(t)=>{try{return typeof openT==='function'?openT(t):true;}catch(e){return true;}};
  const fleet=(t)=>{try{return typeof flota==='function'?flota(t):(t?.flota||t?.fleet||t?.unidad||'-');}catch(e){return t?.flota||'-';}};
  const route=(t)=>{try{return typeof ruta==='function'?ruta(t):(t?.ruta||t?.route||{});}catch(e){return t?.ruta||{};}};
  const driver=(t)=>{try{return typeof driverName==='function'?driverName(t):(t?.chofer||t?.driver||'-');}catch(e){return t?.chofer||'-';}};
  const location=(t)=>{try{return typeof locFull==='function'?locFull(t):(t?.localidad||t?.ubicacion||'-');}catch(e){return '-';}};
  const type=(a)=>{try{return typeof alertTipoV1250==='function'?alertTipoV1250(a):(a?.tipo||a?.type||a?.motivo||a?.nombre||'Alerta');}catch(e){return a?.tipo||a?.type||a?.motivo||'Alerta';}};
  const verified=(t,a,idx)=>{try{if(typeof alertVerifiedV1250==='function')return !!alertVerifiedV1250(t,a,idx);}catch(e){} try{if(typeof isAlertVerified==='function')return !!isAlertVerified(t,a,idx);}catch(e){} return !!(a?.verificada||a?.verificado||a?.vista||a?.verified);};
  function has(v){return v!==undefined&&v!==null&&String(v).trim()!==''&&!/^null$|^undefined$/i.test(String(v).trim());}
  function walk(obj,keys){const q=[obj],seen=new Set(),ks=keys.map(k=>String(k).toLowerCase());while(q.length){const cur=q.shift();if(!cur||typeof cur!=='object'||seen.has(cur))continue;seen.add(cur);for(const [k,v] of Object.entries(cur)){if(ks.includes(String(k).toLowerCase())&&has(v))return String(v).trim();}Object.values(cur).forEach(v=>{if(v&&typeof v==='object')q.push(v);});}return '';}
  function km(a,t){
    const fns=['km215','km211','kmValue213','kmValueV1250'];
    for(const fn of fns){try{if(typeof window[fn]==='function'){const v=window[fn](a,t); if(has(v)&&v!=='-')return v;}}catch(e){}}
    const keys=['km','kms','kilometro','kilómetro','kilometros','kilómetros','kmRuta','km_ruta','rutaKm','ruta_km','kmAlerta','kmControl','progresiva','progresivaKm','kilometraje','ubicacionKm','kmGps','gpsKm','kmActual','puntoKm','hitoKm'];
    let v=walk(a,keys)||walk(t,keys); if(has(v)&&v!=='-')return v;
    try{const blob=JSON.stringify([a,t?.ruta,t?.route,t?.gps,t?.ultimaPosicion,t?.lastPosition]||{}); const m=blob.match(/(?:km|kms|kil[oó]metro|progresiva|punto|hito|ruta)\D{0,28}(-?\d+(?:[\.,]\d+)?)/i)||blob.match(/(-?\d+(?:[\.,]\d+)?)\s*(?:km|kms|kil[oó]metro)/i); return m?m[1].replace(',','.'):'-';}catch(e){return '-';}
  }
  function alertLoc(a,t){try{if(typeof alertLocV1250==='function')return alertLocV1250(a,t);}catch(e){} return a?.localidad||a?.ubicacion||location(t)||'-';}
  function rowsFor(t){return (t?.alerts||[]).map((a,idx)=>({a,idx})).filter(r=>!verified(t,r.a,r.idx)).sort((x,y)=>T(y.a.time||y.a.fecha||y.a.createdAt||y.a.ts)-T(x.a.time||x.a.fecha||x.a.createdAt||x.a.ts));}
  function lastTime(t){try{if(typeof lastGpsTimeV1237==='function')return lastGpsTimeV1237(t);}catch(e){} try{if(typeof lastReportValue==='function')return lastReportValue(t);}catch(e){} return t?.updatedAt||t?.updateAt||t?.start?.time||t?.start;}

  window.trackingCard=function(t){
    const rt=route(t), o=open(t), rows=rowsFor(t);
    const left=`<div class="trackingCardMain218"><div class="trackingCardTop"><div class="trackingFleetTitle">🚚 Flota ${E(fleet(t)||'-')} <span class="trackingEmbTitle">/ 📦 Emb. ${E(t?.embarque||'-')}</span></div><span class="trackingState ${o?'':'closed'}">${o?'Abierto':'Finalizado'}</span></div><div class="trackingData trackingData218"><div><b>Chofer:</b> ${E(driver(t))}</div><div><b>Cliente:</b> ${E(rt?.cliente||'-')}</div><div><b>Origen:</b> ${E(rt?.origen||'-')}</div><div><b>Destino:</b> ${E(rt?.destino||'-')}</div><div><b>Lote/Carga:</b> ${E(t?.lote||t?.carga||'-')}</div><div><b>Inicio:</b> ${F(t?.start?.time||t?.start)}</div><div><b>Cierre:</b> ${o?'-':F(t?.closed?.time||t?.closed)}</div><div><b>Últ. posición:</b> ${E(location(t))}</div><div class="trackingReportLine fullLine"><b>Últ. reporte:</b> ${F(lastTime(t))}</div></div></div>`;
    const right=rows.length?`<aside class="trackingAlertsPane218"><h4>⚠️ Alertas del tránsito</h4>${rows.map(({a})=>`<div class="trackingAlertRow218"><div class="trackingAlertRowTop218"><span>⚠️ <b>${E(type(a))}</b></span><span>${F(a?.time||a?.fecha||a?.createdAt||a?.ts)}</span></div><div class="trackingAlertRowMeta218"><span><b>Km:</b> ${E(km(a,t))}</span><span><b>Localidad:</b> ${E(alertLoc(a,t))}</span></div></div>`).join('')}</aside>`:'';
    return `<div class="trackingCard trackingCard218 ${rows.length?'hasAlerts':'noAlerts'}">${left}${right}</div>`;
  };
  ...
  function rerenderSeguimiento(){try{if(document.querySelector('#mapa.panel.active')&&typeof renderMapa==='function')renderMapa();}catch(e){}}
  document.addEventListener('DOMContentLoaded',()=>{updateVersion();setTimeout(rerenderSeguimiento,250);});
  const oldTab=window.tab; if(typeof oldTab==='function') window.tab=function(id){const r=oldTab.apply(this,arguments);setTimeout(()=>{updateVersion(); if(id==='mapa')rerenderSeguimiento();},0);return r;};
  const oldRefresh=window.refresh; if(typeof oldRefresh==='function') window.refresh=async function(){const r=await oldRefresh.apply(this,arguments);setTimeout(()=>{updateVersion();rerenderSeguimiento();},0);return r;};
})();
