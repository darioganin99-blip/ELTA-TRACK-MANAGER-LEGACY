...
const ELTA_APP_VERSION = "2.0.129";
...
/* ===== V2.0.129 - Nombre oficial del sistema ===== */
(function(){
  const APP_VERSION_V2 = "2.0.129";
...
/* ===== V2.0.129 - Version y menu lateral robustos ===== */
(function(){
...
/* ===== V2.0.129 - Anti-cache y normalizacion final menu/version ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION = VERSION;
...
/* ===== V2.0.129 - Correcciones GPS, clima, alertas y seguimiento ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - Ajustes finales clima, KM y alertas ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - Ajuste final: menu, clima compacto, KM y alertas alineadas ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - Correccion funcional final: menu, clima, KM y alertas compactas ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - Vista Alertas compacta: boton Verificar visible ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - Torre de Control: Últimas alertas formato linea unica ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - Tránsitos: alertas a la derecha y todas las alertas / Seguimiento vuelve a base 2.0.129 ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - Iconos por tipo de alerta en Tránsitos y Seguimiento ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - SOLO vista Clima: diseño aprobado Flotas -> Pasos -> Destinos ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - SOLO vista Clima: formato operativo compacto Flotas -> Pasos -> Destinos ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - SOLO vista Clima: filtro por embarque y orden Flotas -> Pasos -> Destinos ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - SOLO vista Clima: filtro embarques activos + layout compacto corregido ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - SOLO vista Clima: tarjetas compactas + datos operativos + paso real oficial ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - SOLO vista Clima: actualizar funcional, tarjetas compactas y Paso oficial ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - SOLO vista Clima: columnas Flotas | Paso+Destinos, tarjetas compactas ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - SOLO vista Clima: reorganizacion final compacta + paso oficial bajo demanda ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - Clima: layout compacto definitivo y Paso con consulta online ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - Vista Alertas: resumen con graficos + tabla compacta funcional ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - Alertas: graficos en una linea, campana completa, contador funcional ===== */
(function(){
  const VERSION_ALERTS_2035 = "2.0.129";
  const $ = (id)=>document.getElementById(id);
...
/* ===== V2.0.129 - Menu lateral: badge y efecto de campana segun pendientes ===== */
(function(){
...
/* ===== V2.0.129 - Normalizacion final del menu lateral ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - Refuerzo campana menu lateral ===== */
(function(){
...
/* ===== V2.0.129 - contador real y badge compacto de menu ===== */
(function(){
  const APP_VERSION_MENU_2039 = '2.0.129';
...
/* ===== V2.0.129 - Menu base limpio: normalizadores anteriores removidos ===== */
/* ===== V2.0.129 - Nuevo modulo Embarques ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - Embarques: combos desde colecciones Firestore y estructura real ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== v2.0.129 - PDF profesional con logo original ELTA para Vista Tránsitos ===== */
(function(){
  const APP_VERSION_PDF = '2.0.129';
  ...
  document.querySelectorAll('.loginFooter span, .headerTitle span').forEach(el=>{if(/Versi[oó]n/i.test(el.textContent||''))el.textContent='Versión 2.0.129';});
})();


/* ===== V2.0.129 - Fix botones Actualizar global + version unificada ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - Configuracion redisenada sin JSON y sin pestana Embarques ===== */
(function(){
  const APP_VERSION_CFG = "2.0.129";
...
/* ===== V2.0.129 - SOLO vista Clima: filtro embarque corrige destino exacto del embarque ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - FIX DEFINITIVO SOLO CLIMA: filtro por embarque trae destino/paso aunque no haya flota ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - Datos reales del usuario logueado en sidebar ===== */
(function(){
  const APP_VERSION_LOGGED_USER_FIX = "2.0.129";
...
/* ===== V2.0.129 - Clima compacto + PDF OEA + correcciones puntuales ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  function $id(id){return document.getElementById(id);}
...
/* ===== V2.0.129 - Version + flotas del combo desde usuarios role=flota ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - FIX DEFINITIVO: version, clima compacto y combo flotas desde Firebase ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - Embarques: boton Ver cargas en Agregar carga ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - Recuperar password, ajustes Configuracion, Embarques y KM Alertas ===== */
(function(){
  const APP_VERSION_FIX='2.0.129';
...
/* ===== V2.0.129 - Password visible en Configuracion y version unificada ===== */
(function(){
  const APP_VERSION_273='2.0.129';
...
/* ===== V2.0.129 - Version lock definitivo ===== */
(function(){
  const VERSION = '2.0.129';
  window.ELTA_APP_VERSION = VERSION;
  window.APP_VERSION_V2 = VERSION;
...
/* ===== V2.0.129 - Embarques: eliminar registro con confirmacion ===== */
(function(){
  const VERSION='2.0.129';
  const S=v=>String(v==null?'':v).trim();
  const E=v=>S(v).replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
...
/* ===== V2.0.129 - Embarques: papelera sin fondo ===== */
(function(){
  const css=document.createElement('style');
...
/* ===== V2.0.129 - MENU LATERAL ESTABLE: sin intervalos ni reescrituras repetidas ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - Entrega por flota, compacta y con aduanas por pais destino ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
  ...
  function countryCode(v){
    const c=countryOf(v);
    return c==='argentina'?'AR':c==='paraguay'?'PY':c==='uruguay'?'UY':c==='chile'?'CL':'--';
  }
...
/* ===== V2.0.129 - Entrega: flota, banderas por pais, aduanas por destino, compacto ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - ajuste final SOLO Entregas: menú estable, timeline sin superposición ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION; window.APP_VERSION_V2=VERSION;
  function setV(){
...
/* ===== V2.0.129 - FIX FINAL VERSION Y MENU ESTABLE ===== */
(function(){
  const VERSION='2.0.129';
...
/* ===== V2.0.129 - SOLO ENTREGAS: icono único y tarjeta según boceto ===== */
(function(){
  const VERSION='2.0.129';
...
/* ===== V2.0.129 - SOLO ENTREGAS: tarjeta compacta, camion reducido, banderas icono ===== */
(function(){
  const VERSION='2.0.129';
  function applyVersion(){
    window.ELTA_APP_VERSION=VERSION; window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - SOLO ENTREGAS: tarjeta igual al boceto, hitos sobre linea, camion chico, banderas reales ===== */
(function(){
  const VERSION='2.0.129';
...
/* ===== V2.0.129 - SOLO ENTREGAS: tarjeta reconstruida igual boceto, sin tocar Embarques ===== */
(function(){
  const VERSION='2.0.129';
  function setVer(){
...
/* ===== V2.0.129 - CORRECCION FINAL SOLO ENTREGAS: tarjeta igual al boceto, sin tocar Embarques ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - SOLO Entregas: tarjeta compacta final y 2 banderas reales ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION; window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - SOLO Entregas: contenedor compacto, hitos sobre linea, sin checks, 2 banderas ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION; window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - SOLO Entregas: correccion real tarjeta compacta, banderas y timeline ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION;
  window.APP_VERSION_V2=VERSION;
...
/* ===== V2.0.129 - ENTREGAS: restaurar diseño 2.0.118 + banderas reales sin tocar menú ===== */
(function(){
  const VERSION='2.0.129';
  window.ELTA_APP_VERSION=VERSION; window.APP_VERSION_V2=VERSION;
  function setVersion(){
  ...
  function countryCode(txt){
    const t=norm(txt);
    if(/\bbr\b|brasil|brazil/.test(t)) return 'br';
    if(/\bpy\b|paraguay|falcon|falcon|asuncion|clorinda/.test(t)) return 'py';
    if(/\buy\b|uruguay|montevideo|nordex|fray bentos|colonia/.test(t)) return 'uy';
    if(/\bcl\b|chile|los andes|santiago|stli|toyota chile|libertadores/.test(t)) return 'cl';
    if(/\bar\b|argentina|zarate|cordoba|buenos aires|caba|clz|clc/.test(t)) return 'ar';
    return '';
  }
  function codesFromCard(card){
    const text=card.textContent||'';
    const labels=[...card.querySelectorAll('.entregaStep b')].map(x=>x.textContent||'').filter(Boolean);
    let from=countryCode(labels[0]||text)||'ar';
    let to=countryCode(labels[labels.length-1]||'')||countryCode(text);
    if(!to || to===from){
      const all=norm(text);
      if(all.includes('paraguay')||all.includes('falcon')) to='py';
      else if(all.includes('uruguay')||all.includes('nordex')) to='uy';
      else if(all.includes('brasil')||all.includes('brazil')) to='br';
      else if(all.includes('chile')||all.includes('los andes')||all.includes('toyota chile')) to='cl';
      else to='cl';
    }
    return {from,to};
  }
  function flagHTML(c){return '<span class="eltaRealFlag flag-'+c+'" aria-label="'+c.toUpperCase()+'"></span>';}
  function applyFlags(){
    document.querySelectorAll('#entrega .entregaCard').forEach(card=>{
      const box=card.querySelector('.entregaFlags'); if(!box) return;
      const c=codesFromCard(card);
      box.innerHTML=flagHTML(c.from)+'<span class="eltaRealArrow">›</span>'+flagHTML(c.to);
      box.setAttribute('data-route',c.from+'-'+c.to);
    });
  }
  function css(){
    if(document.getElementById('entregas-129-flags-css')) return;
    const st=document.createElement('style'); st.id='entregas-129-flags-css';
    st.textContent=`
      #entrega .entregaFlags{width:128px!important;min-width:128px!important;max-width:128px!important;height:56px!important;padding:8px 12px!important;border:1px solid rgba(148,163,184,.28)!important;border-radius:14px!important;background:rgba(255,255,255,.035)!important;display:flex!important;align-items:center!important;justify-content:center!important;gap:13px!important;box-sizing:border-box!important;overflow:hidden!important;justify-self:end!important;}
      #entrega .eltaRealArrow{font-size:30px!important;line-height:1!important;color:#7cff63!important;font-weight:1000!important;margin-top:-1px!important;}
      #entrega .eltaRealFlag{position:relative!important;display:inline-block!important;width:36px!important;height:36px!important;border-radius:50%!important;overflow:hidden!important;flex:0 0 36px!important;border:1px solid rgba(255,255,255,.9)!important;box-shadow:0 2px 8px rgba(0,0,0,.42)!important;background:#e5e7eb!important;}
      #entrega .flag-ar{background:linear-gradient(to bottom,#75bdf2 0 33%,#fff 33% 66%,#75bdf2 66% 100%)!important;}#entrega .flag-ar:after{content:''!important;position:absolute!important;left:50%!important;top:50%!important;width:7px!important;height:7px!important;background:#f6b71d!important;border-radius:50%!important;transform:translate(-50%,-50%)!important;}
      #entrega .flag-cl{background:linear-gradient(to bottom,#fff 0 50%,#d52b1e 50% 100%)!important;}#entrega .flag-cl:before{content:''!important;position:absolute!important;left:0!important;top:0!important;width:45%!important;height:50%!important;background:#1d4fa3!important;}#entrega .flag-cl:after{content:'★'!important;position:absolute!important;left:5px!important;top:1px!important;color:#fff!important;font-size:8px!important;line-height:1!important;}
      #entrega .flag-py{background:linear-gradient(to bottom,#d52b1e 0 33%,#fff 33% 66%,#21468b 66% 100%)!important;}#entrega .flag-py:after{content:''!important;position:absolute!important;left:50%!important;top:50%!important;width:6px!important;height:6px!important;background:#d4af37!important;border-radius:50%!important;transform:translate(-50%,-50%)!important;}
      #entrega .flag-uy{background:repeating-linear-gradient(to bottom,#fff 0 4px,#fff 4px 7px,#58a7e6 7px 10px)!important;}#entrega .flag-uy:before{content:''!important;position:absolute!important;left:0!important;top:0!important;width:46%!important;height:46%!important;background:#fff!important;}#entrega .flag-uy:after{content:''!important;position:absolute!important;left:9px!important;top:9px!important;width:7px!important;height:7px!important;border-radius:50%!important;background:#f6b71d!important;}
      #entrega .flag-br{background:#229e45!important;}#entrega .flag-br:before{content:''!important;position:absolute!important;left:50%!important;top:50%!important;width:24px!important;height:24px!important;background:#f9e64f!important;transform:translate(-50%,-50%) rotate(45deg)!important;}#entrega .flag-br:after{content:''!important;position:absolute!important;left:50%!important;top:50%!important;width:13px!important;height:13px!important;background:#2b55a2!important;border-radius:50%!important;transform:translate(-50%,-50%)!important;}
      #entrega .entregaCard:nth-of-type(even){background:linear-gradient(145deg,rgba(28,52,72,.92),rgba(18,39,58,.90))!important;border-color:rgba(148,163,184,.24)!important;}
      #entrega .entregaToolbar{display:grid!important;grid-template-columns:repeat(5,minmax(0,1fr))!important;gap:12px!important;padding:16px 18px!important;}
      #entrega .entregaToolbar label span{display:none!important;}#entrega .entregaToolbar select{height:48px!important;padding:0 42px 0 18px!important;border-radius:12px!important;font-size:16px!important;font-weight:850!important;background:#243244!important;color:#f8fafc!important;border:1px solid rgba(148,163,184,.38)!important;}
    `;
    document.head.appendChild(st);
  }
  function run(){css(); setVersion(); applyFlags();}
  const oldRender=window.renderEntrega;
  if(typeof oldRender==='function'&&!oldRender.__flags129){
    const f=async function(){const r=await oldRender.apply(this,arguments); setTimeout(run,0); return r;};
    f.__flags129=true; window.renderEntrega=f;
  }
  const oldTab=window.tab;
  if(typeof oldTab==='function'&&!oldTab.__flags129){
    const f=function(id){const r=oldTab.apply(this,arguments); if(id==='entrega')setTimeout(run,80); else setTimeout(setVersion,40); return r;};
    f.__flags129=true; window.tab=f;
  }
...
/* ===== V2.0.130 - ENTREGAS ESTABLE FINAL: render unico, banderas reales, filtros compactos ===== */
(function(){
  const VERSION='2.0.130';
  window.ELTA_APP_VERSION=VERSION;
  ...
  function countryCode(v){
    const s=low(v);
    if(/\b(py|paraguay)\b/.test(s)||s.includes('falcón')||s.includes('falcon')||s.includes('asuncion')||s.includes('asunción')) return 'PY';
    if(/\b(uy|uruguay)\b/.test(s)||s.includes('montevideo')||s.includes('fray bentos')||s.includes('nordex')) return 'UY';
    if(/\b(cl|chile)\b/.test(s)||s.includes('los andes')||s.includes('libertadores')||s.includes('stli')||s.includes('toyota chile')) return 'CL';
    if(/\b(br|brasil|brazil)\b/.test(s)||s.includes('sao ')||s.includes('são ')||s.includes('curitiba')) return 'BR';
    return 'AR';
  }
  function flagHtml(code){return '<span class="eltaFlag eltaFlag-'+code.toLowerCase()+'" title="'+code+'"></span>';}
  function fmt(v){
    try{ if(!v) return '-'; if(v.toDate) v=v.toDate(); const d=new Date(v); if(isNaN(d)) return String(v); return d.toLocaleString('es-AR',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'}); }catch(e){ return String(v||'-'); }
  }
  function elapsed(v){
    try{ if(!v) return '-'; if(v.toDate) v=v.toDate(); const d=new Date(v); if(isNaN(d)) return '-'; let ms=Date.now()-d.getTime(); if(ms<0) ms=0; const h=Math.floor(ms/3600000), day=Math.floor(h/24), hh=h%24, m=Math.floor((ms%3600000)/60000); return (day?day+' d ':'')+hh+' h '+m+' m'; }catch(e){return '-';}
  }
  function statusFor(t,e){
    const s=low(pick(t,['estado','status'])||pick(e,['estado','status']));
    if(s.includes('final')||s.includes('cerr')||t?.closed||t?.cierre) return {txt:'FINALIZADO',cls:'done'};
    if(s.includes('aduana')) return {txt:'EN ADUANA',cls:'customs'};
    if(s.includes('program')) return {txt:'PROGRAMADO',cls:'planned'};
    return {txt:'EN TRÁNSITO',cls:'open'};
  }
  function embNum(e){return clean(pick(e,['numero','embarque','idEmbarque','codigo','id','_docId']));}
  function trEmb(t){return clean(pick(t,['embarque','emb','embarqueId','numeroEmbarque','numero']));}
  function trFleet(t){return clean(pick(t,['flota','fleet','usuario','user']) || (t?.driver&&t.driver.flota));}
  function routeOf(t){ try{ if(typeof window.ruta==='function') return window.ruta(t)||{}; }catch(e){} return t?.ruta||t?.route||{}; }
  function lastTime(t,e){return pick(t,['updatedAt','lastUpdate','lastReport','ultimoReporte','fechaActualizacion','fecha','start']) || pick(e,['fecha','fechaHora','createdAt','start']);}
  function stagesFor(row){
    const dest=countryCode(row.destino);
    let mid=[];
    if(dest==='PY') mid=['Clorinda','Puerto José Falcón','Deposito Fiscal Paraguay'];
    else if(dest==='CL') mid=['Uspallata','Los Andes'];
    else if(dest==='UY') mid=['Fray Bentos'];
    else if(dest==='BR') mid=['Paso de los Libres','Uruguaiana'];
    else mid=[];
    const out=[row.origen||'Origen',...mid,row.destino||'Destino'].filter(Boolean);
    return out.slice(0,5);
  }
  function progressIndex(row,stages){
    const s=low(row.transit?.estado||row.emb?.estado||'');
    if(s.includes('final')||s.includes('cerr')) return stages.length-1;
    if(s.includes('aduana')) return Math.min(1,stages.length-1);
    return Math.min(1,stages.length-1);
  }
  function buildRows(){
    const embs=arr('embarques'), trs=arr('trs');
    const embMap=new Map(); embs.forEach(e=>{const n=embNum(e); if(n) embMap.set(String(n),e);});
    const groups=new Map();
    trs.forEach(t=>{
      const fl=trFleet(t); if(!fl) return;
      const n=trEmb(t); const e=embMap.get(String(n))||{}; const rt=routeOf(t)||{};
      if(!groups.has(fl)) groups.set(fl,{flota:fl,items:[]});
      groups.get(fl).items.push({t,e,rt,numero:n});
    });
    embs.forEach(e=>{
      const fls=clean(pick(e,['flota','flotas','fleet'])); if(!fls) return;
      fls.split(',').map(x=>x.trim()).filter(Boolean).forEach(fl=>{
        if(!groups.has(fl)) groups.set(fl,{flota:fl,items:[]});
        if(!groups.get(fl).items.length) groups.get(fl).items.push({t:{},e,rt:{},numero:embNum(e)});
      });
    });
    return Array.from(groups.values()).map(g=>{
      g.items.sort((a,b)=>String(lastTime(b.t,b.e)||b.numero).localeCompare(String(lastTime(a.t,a.e)||a.numero),'es',{numeric:true}));
      const it=g.items[0]||{}, t=it.t||{}, e=it.e||{}, rt=it.rt||{};
      const nums=[...new Set(g.items.map(x=>x.numero).filter(Boolean))];
      return {flota:g.flota, numero:it.numero||embNum(e), embarques:nums, cliente:clean(pick(e,['cliente','client'])||pick(rt,['cliente'])||pick(t,['cliente'])), origen:clean(pick(e,['origen','origin'])||pick(rt,['origen'])||pick(t,['origen'])||'Centro Logístico Zárate'), destino:clean(pick(e,['destino','destination'])||pick(rt,['destino'])||pick(t,['destino'])||'Destino'), fecha:lastTime(t,e), transit:t, emb:e, estado:statusFor(t,e).txt};
    }).sort((a,b)=>String(a.flota).localeCompare(String(b.flota),'es',{numeric:true}));
  }
  function ensureMenuStable(){
    const nav=$('.sideNav'); if(!nav) return;
    let entrega=nav.querySelector('button[data-menu-id="entrega"]') || $$('.sideNav button').find(b=>(b.getAttribute('onclick')||'').includes('entrega'));
    if(!entrega){ entrega=document.createElement('button'); nav.appendChild(entrega); }
    entrega.type='button'; entrega.dataset.menuId='entrega'; entrega.setAttribute('onclick',"tab('entrega')");
    entrega.innerHTML='<span class="menuIcon">🏁</span><span class="menuText">Entregas</span>';
    const emb=$$('.sideNav button').find(b=>(b.getAttribute('onclick')||'').includes('embarques'));
    if(emb && emb.nextSibling!==entrega) emb.parentNode.insertBefore(entrega, emb.nextSibling);
  }
  function ensurePanel(){
    let sec=$('#entrega');
    if(!sec){ sec=document.createElement('section'); sec.id='entrega'; sec.className='panel entregaPanel'; const emb=$('#embarques'); (emb?.parentNode||$('.dashboardShell')||document.body).insertBefore(sec, emb?emb.nextSibling:null); }
    if(sec.dataset.v130==='ok') return sec;
    sec.innerHTML='<div class="sectionTitle entregaHeader"><div><h2>Entregas</h2><p>Seguimiento de flotas en tránsito</p></div><button type="button" class="entregaRefreshBtn" id="entregaRefreshBtn">Actualizar</button></div><div class="entregaToolbar glassPanel"><select id="entregaFiltroCliente"><option value="">Todos los clientes</option></select><select id="entregaFiltroEmbarque"><option value="">Embarque</option></select><select id="entregaFiltroDestino"><option value="">Todos los destinos</option></select><select id="entregaFiltroFlota"><option value="">Todas las flotas</option></select><select id="entregaFiltroEstado"><option value="">Todos</option><option value="en tránsito">EN TRÁNSITO</option><option value="en aduana">EN ADUANA</option><option value="programado">PROGRAMADO</option><option value="finalizado">FINALIZADO</option></select></div><div id="entregaCards" class="entregaCards"></div>';
    sec.dataset.v130='ok';
    ['entregaFiltroCliente','entregaFiltroEmbarque','entregaFiltroDestino','entregaFiltroFlota','entregaFiltroEstado'].forEach(id=>{$('#'+id)?.addEventListener('change',()=>window.renderEntrega(false));});
    $('#entregaRefreshBtn')?.addEventListener('click',()=>window.renderEntrega(true));
    return sec;
  }
  function fillSelect(id, vals, placeholder){
    const el=$('#'+id); if(!el) return; const old=el.value;
    el.innerHTML='<option value="">'+placeholder+'</option>'+Array.from(new Set(vals.filter(Boolean))).sort((a,b)=>String(a).localeCompare(String(b),'es',{numeric:true})).map(v=>'<option value="'+esc(v)+'">'+esc(v)+'</option>').join('');
    if(Array.from(el.options).some(o=>o.value===old)) el.value=old;
  }
  function updateFilters(rows){
    fillSelect('entregaFiltroCliente', rows.map(r=>r.cliente), 'Todos los clientes');
    fillSelect('entregaFiltroEmbarque', rows.flatMap(r=>r.embarques.length?r.embarques:[r.numero]).filter(Boolean), 'Embarque');
    fillSelect('entregaFiltroDestino', rows.map(r=>r.destino), 'Todos los destinos');
    fillSelect('entregaFiltroFlota', rows.map(r=>r.flota), 'Todas las flotas');
  }
  function passes(row){
    const fc=low($('#entregaFiltroCliente')?.value), fe=low($('#entregaFiltroEmbarque')?.value), fd=low($('#entregaFiltroDestino')?.value), ff=low($('#entregaFiltroFlota')?.value), fs=low($('#entregaFiltroEstado')?.value);
    if(fc && !low(row.cliente).includes(fc)) return false;
    if(fe && !row.embarques.concat([row.numero]).some(x=>low(x).includes(fe))) return false;
    if(fd && !low(row.destino).includes(fd)) return false;
    if(ff && !low(row.flota).includes(ff)) return false;
    if(fs && !low(row.estado).includes(fs)) return false;
    return true;
  }
  function card(row,idx){
    const st=statusFor(row.transit,row.emb), updated=fmt(lastTime(row.transit,row.emb)), el=elapsed(lastTime(row.transit,row.emb));
    const stages=stagesFor(row), pi=progressIndex(row,stages), pct=stages.length>1 ? (pi/(stages.length-1))*100 : 0;
    const from=countryCode(row.origen), to=countryCode(row.destino);
    const next=stages[Math.min(pi+1,stages.length-1)]||'-';
    return '<article class="entregaCard '+(idx%2?'entregaCardAlt':'')+'"><div class="entregaTop"><div class="entregaFleet"><span class="entregaFleetIcon"></span><div><h3>Flota '+esc(row.flota)+'</h3><span class="entregaStatus '+st.cls+'">'+esc(st.txt)+'</span><small>Emb. '+esc((row.embarques.length?row.embarques.join(', '):row.numero)||'-')+'</small></div></div><div class="entregaMeta"><span><i>📅</i>Última act.</span><b>'+esc(updated)+'</b></div><div class="entregaMeta"><span><i>⏱</i>Tiempo transcurrido</span><b>'+esc(el)+'</b></div><div class="entregaMeta"><span><i>🚩</i>Próximo hito</span><b>'+esc(next)+'</b></div><div class="entregaMeta"><span>Estado</span><b class="state '+st.cls+'">'+esc(st.txt)+'</b></div><div class="entregaFlags">'+flagHtml(from)+'<i>›</i>'+flagHtml(to)+'</div></div><div class="entregaTimeline"><div class="entregaLine"><i style="width:'+Math.max(0,Math.min(100,pct))+'%"></i></div>'+stages.map((s,i)=>{const left=stages.length>1?(i/(stages.length-1))*100:0; return '<div class="entregaStep '+(i<=pi?'done':'')+' '+(i===pi?'current':'')+'" style="left:'+left+'%"><b>'+esc(s)+'</b><em>'+esc(i===0?fmt(row.fecha):(i===pi?updated:'-'))+'</em></div>';}).join('')+'<div class="entregaVehicle" style="left:'+Math.max(0,Math.min(100,pct))+'%"><img class="entregaCarrierImg" src="assets/camion-automobilero.png" alt="Camión automobilero"></div></div><div class="entregaBottom"><span><b>Actualizado:</b> '+esc(updated)+'</span><span><b>Tiempo transcurrido:</b> '+esc(el)+'</span><span><b>Próximo hito:</b> '+esc(next)+'</span><span><b>Estado:</b> '+esc(st.txt)+'</span></div></article>';
  }
  function injectCss(){
    // neutraliza estilos viejos solo de Entregas
    $$('style[id*="entrega"],style[id*="Entregas"]').forEach(s=>{ if(s.id!=='entrega-v20130-final-css') s.remove(); });
    if($('#entrega-v20130-final-css')) return;
    const st=document.createElement('style'); st.id='entrega-v20130-final-css'; st.textContent=`
      #entrega .entregaHeader{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px}#entrega .entregaRefreshBtn{height:42px;border-radius:13px;border:1px solid rgba(111,255,95,.45);background:#26752d;color:#eaffea;font-weight:900;padding:0 22px}
      #entrega .entregaToolbar{display:grid!important;grid-template-columns:repeat(5,minmax(150px,1fr))!important;gap:12px!important;padding:10px 12px!important;margin-bottom:12px!important;border-radius:16px!important;background:rgba(12,29,46,.88)!important;border:1px solid rgba(148,163,184,.22)!important}.entregaToolbar select{width:100%!important;height:46px!important;border-radius:13px!important;border:1px solid rgba(148,163,184,.32)!important;background:#243347!important;color:#f8fafc!important;padding:0 18px!important;font-size:16px!important;font-weight:900!important;outline:none!important}.entregaCards{display:flex;flex-direction:column;gap:10px!important}
      #entrega .entregaCard{padding:12px 14px!important;border-radius:16px!important;background:linear-gradient(135deg,rgba(7,23,38,.98),rgba(5,16,29,.96))!important;border:1px solid rgba(148,163,184,.24)!important;box-shadow:0 12px 28px rgba(0,0,0,.24)!important;overflow:hidden!important}.entregaCardAlt{background:linear-gradient(135deg,rgba(26,48,67,.98),rgba(18,36,54,.96))!important;border-color:rgba(148,180,210,.34)!important}
      #entrega .entregaTop{display:grid!important;grid-template-columns:1.24fr 1fr 1fr .9fr .78fr 132px!important;gap:10px!important;align-items:center!important;padding:0 0 10px!important;border-bottom:1px solid rgba(148,163,184,.14)!important}.entregaFleet{display:flex!important;align-items:center!important;gap:10px!important}.entregaFleetIcon{width:46px!important;height:46px!important;border-radius:50%!important;border:2px solid #37b342!important;background:rgba(34,197,94,.10) url('assets/camion-automobilero.png') center/34px auto no-repeat!important;flex:0 0 46px!important}.entregaFleet h3{font-size:22px!important;line-height:1!important;margin:0 0 5px!important;color:#f8fafc!important}.entregaFleet small{display:block;font-size:12px!important;color:#cbd5e1!important;margin-top:5px!important}.entregaStatus{display:inline-block!important;font-size:11px!important;line-height:1!important;padding:4px 10px!important;border-radius:8px!important;background:#17642c!important;color:#9dff7e!important;font-weight:900!important}
      #entrega .entregaMeta{border-left:1px solid rgba(148,163,184,.16)!important;padding-left:10px!important;min-width:0!important;text-align:center!important}.entregaMeta span{display:flex!important;align-items:center!important;justify-content:center!important;gap:6px!important;font-size:11px!important;color:#cbd5e1!important;margin-bottom:5px!important}.entregaMeta span i{font-style:normal!important;color:#7cff63!important}.entregaMeta b{display:block!important;font-size:13px!important;line-height:1.1!important;color:#f8fafc!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important}.entregaMeta b.state{font-size:14px!important;color:#7cff63!important}
      #entrega .entregaFlags{height:64px!important;min-width:126px!important;border:1px solid rgba(148,163,184,.26)!important;background:rgba(255,255,255,.035)!important;border-radius:14px!important;display:flex!important;align-items:center!important;justify-content:center!important;gap:14px!important}.entregaFlags i{font-style:normal!important;color:#7cff63!important;font-size:26px!important;font-weight:900!important;line-height:1!important}.eltaFlag{width:38px!important;height:38px!important;border-radius:50%!important;display:inline-block!important;position:relative!important;overflow:hidden!important;box-shadow:inset 0 0 0 1px rgba(255,255,255,.7),0 2px 8px rgba(0,0,0,.38)!important;flex:0 0 38px!important}.eltaFlag-ar{background:linear-gradient(#79bff3 0 33%,#fff 33% 66%,#79bff3 66% 100%)!important}.eltaFlag-ar:after{content:'';position:absolute;left:50%;top:50%;width:7px;height:7px;background:#f4b000;border-radius:50%;transform:translate(-50%,-50%)}.eltaFlag-cl{background:linear-gradient(90deg,#1d4fa3 0 43%,transparent 43%),linear-gradient(#fff 0 50%,#d52b1e 50% 100%)!important}.eltaFlag-cl:after{content:'★';position:absolute;left:6px;top:2px;color:#fff;font-size:8px}.eltaFlag-py{background:linear-gradient(#d52b1e 0 33%,#fff 33% 66%,#234aa0 66% 100%)!important}.eltaFlag-py:after{content:'';position:absolute;left:50%;top:50%;width:6px;height:6px;background:#c9a84f;border-radius:50%;transform:translate(-50%,-50%)}.eltaFlag-uy{background:repeating-linear-gradient(to bottom,#fff 0 4px,#fff 4px 7px,#58a7e6 7px 10px)!important}.eltaFlag-uy:after{content:'';position:absolute;left:7px;top:7px;width:7px;height:7px;background:#f4b000;border-radius:50%}.eltaFlag-br{background:#159447!important}.eltaFlag-br:before{content:'';position:absolute;left:50%;top:50%;width:22px;height:22px;background:#f7d13d;transform:translate(-50%,-50%) rotate(45deg)}.eltaFlag-br:after{content:'';position:absolute;left:50%;top:50%;width:12px;height:12px;background:#21468b;border-radius:50%;transform:translate(-50%,-50%)}
      #entrega .entregaTimeline{position:relative!important;height:160px!important;margin:10px 0 8px!important;padding:0 44px!important;border-radius:14px!important;background:rgba(255,255,255,.012)!important;overflow:visible!important}.entregaLine{position:absolute!important;left:74px!important;right:74px!important;top:70px!important;height:3px!important;border-radius:8px!important;background:rgba(226,232,240,.78)!important;overflow:hidden!important;z-index:2!important}.entregaLine i{display:block!important;height:100%!important;border-radius:8px!important;background:linear-gradient(90deg,#48e35b,#9bd814,#f4a000)!important}.entregaStep{position:absolute!important;top:32px!important;width:150px!important;min-width:0!important;text-align:center!important;transform:translateX(-50%)!important;z-index:4!important}.entregaStep b{display:block!important;margin:0 0 30px!important;font-size:11.5px!important;line-height:1.1!important;text-transform:uppercase!important;color:#f8fafc!important;text-shadow:0 1px 3px rgba(0,0,0,.6)!important}.entregaStep em{display:block!important;margin-top:16px!important;color:#cbd5e1!important;font-size:10.5px!important;font-style:normal!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important}.entregaStep:after{content:''!important;position:absolute!important;left:50%!important;top:32px!important;width:22px!important;height:22px!important;border-radius:50%!important;background:#edf2f7!important;border:4px solid #64748b!important;box-shadow:0 0 0 3px rgba(15,23,42,.9)!important;transform:translate(-50%,-50%)!important;box-sizing:border-box!important;z-index:5!important}.entregaStep.done:after{border-color:#3dd34a!important}.entregaStep.current:after{border-color:#3dd34a!important}.entregaVehicle{position:absolute!important;top:118px!important;transform:translateX(-50%)!important;z-index:6!important;width:42px!important;height:22px!important;display:flex!important;align-items:center!important;justify-content:center!important;pointer-events:none!important}.entregaCarrierImg{width:42px!important;max-width:42px!important;height:auto!important;display:block!important;background:transparent!important}.entregaTimeline:after{content:'';position:absolute;left:74px;right:74px;top:128px;border-top:2px dashed rgba(148,163,184,.54);z-index:1}
      #entrega .entregaBottom{display:grid!important;grid-template-columns:repeat(4,1fr)!important;gap:10px!important;margin-top:8px!important;padding:8px 12px!important;border-radius:12px!important;background:rgba(255,255,255,.035)!important;border:1px solid rgba(148,163,184,.15)!important;color:#dbeafe!important}.entregaBottom span{font-size:11.5px!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important}.entregaBottom b{font-size:11.5px!important;color:#8cff6f!important}
      @media(max-width:1250px){#entrega .entregaToolbar{grid-template-columns:1fr 1fr!important}#entrega .entregaTop{grid-template-columns:1fr 1fr!important}#entrega .entregaBottom{grid-template-columns:1fr!important}}
    `; document.head.appendChild(st);
  }
  async function render(force=false){
    ensureMenuStable(); ensurePanel(); injectCss(); setVersion();
    if(force && typeof window.refresh==='function'){ try{ await window.refresh(); }catch(e){} }
    const rows=buildRows(); updateFilters(rows);
    const filtered=rows.filter(passes);
    const box=$('#entregaCards'); if(box) box.innerHTML=filtered.length?filtered.map(card).join(''):'<div class="glassPanel entregaEmpty">No hay flotas para el filtro seleccionado.</div>';
    injectCss(); setVersion();
  }
  window.renderEntrega=render;
  const originalTab=window.tab;
  window.tab=function(id){
    $$('.panel').forEach(p=>p.classList.remove('active')); const panel=$('#'+id); if(panel) panel.classList.add('active');
    $$('.sideNav button').forEach(b=>b.classList.toggle('active',(b.getAttribute('onclick')||'').includes("'"+id+"'") || (id==='entrega'&&b.dataset.menuId==='entrega')));
    if(id==='entrega'){ ensureMenuStable(); ensurePanel(); render(false); return; }
    if(typeof originalTab==='function') return originalTab.apply(this,arguments);
  };
  function init(){ensureMenuStable(); ensurePanel(); injectCss(); setVersion(); if($('#entrega')?.classList.contains('active')) render(false);}
