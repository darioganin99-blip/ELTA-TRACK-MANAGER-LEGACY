...
/* ===== V2.0.138 - SOLO ENTREGAS: estabilizar menu, sin re-render al re-tocar ===== */
(function(){
  const VERSION='2.0.138';
  const ENTREGA_ICON='🚛';
  function setVersion138(){
    window.ELTA_APP_VERSION=VERSION;
    window.APP_VERSION_V2=VERSION;
  ...
  function entregaBtn(){
    return document.querySelector('button[data-menu-id="entrega"]') ||
      [...document.querySelectorAll('.sideNav button')].find(b=>(b.getAttribute('onclick')||'').includes("tab('entrega')") || (b.getAttribute('onclick')||'').includes('tab("entrega")'));
  }
  function lockEntregaMenu(){
    const btn=entregaBtn();
    if(!btn) return;
    ...
    if(ic.textContent!==ENTREGA_ICON) ic.textContent=ENTREGA_ICON;
    if(tx.textContent!=='Entregas') tx.textContent='Entregas';
    btn.title='Entregas';
  }
  const previousTab=window.tab;
  if(typeof previousTab==='function' && !previousTab.__v20138EntregaStable){
    window.tab=function(id){
      if(id==='entrega'){
        const panel=document.getElementById('entrega');
        const alreadyActive=panel && panel.classList.contains('active');
        document.querySelectorAll('.sideNav button').forEach(b=>b.classList.remove('active'));
        document.querySelectorAll('.panel').forEach(p=>p.classList.remove('active'));
        const btn=entregaBtn();
        ...
        lockEntregaMenu();
        setVersion138();
        if(!alreadyActive && typeof window.renderEntrega==='function'){
          const cards=document.getElementById('entregaCards');
          if(!cards || !cards.children.length) setTimeout(()=>window.renderEntrega(false),0);
        }
        return false;
      }
      const r=previousTab.apply(this,arguments);
      lockEntregaMenu();
      setVersion138();
      return r;
    };
    window.tab.__v20138EntregaStable=true;
  }
  function boot(){lockEntregaMenu();setVersion138();}
  document.addEventListener('DOMContentLoaded',boot);
  window.addEventListener('load',()=>{boot();setTimeout(boot,100);setTimeout(boot,500);});
  const nav=document.querySelector('.sideNav');
  if(nav){
    let pending=false;
    new MutationObserver(()=>{
      if(pending) return;
      pending=true;
      requestAnimationFrame(()=>{pending=false;lockEntregaMenu();});
    }).observe(nav,{childList:true,subtree:true,characterData:true,attributes:true,attributeFilter:['class','onclick','data-menu-id']});
  }
  boot();
})();
