...
/* ===== V2.0.133 - FIX SEGURO MENU ENTREGAS: sin duplicado, sin flicker, sin romper inicio ===== */
(function(){
  ...
  function setVersion133(){
    try{window.ELTA_APP_VERSION=VERSION; window.APP_VERSION=VERSION; window.APP_VERSION_V2=VERSION;}catch(e){}
    document.querySelectorAll('span,small,p,div,footer').forEach(el=>{
      if(el && el.childElementCount===0 && /Versi[oó]n\s+\d+\.\d+\.\d+/i.test(el.textContent||'')){
        el.textContent=(el.textContent||'').replace(/Versi[oó]n\s+\d+\.\d+\.\d+/gi,'Versión '+VERSION);
      }
    });
  }
  function btnId(btn){
    ...
    const m=oc.match(/tab\(['\"]([^'\"]+)['\"]\)/);
    return btn?.dataset?.menuId || (m?m[1]:'');
  }
  function setButton(btn,id,icon,text){
    if(!btn) return;
    const wanted=`<span class="menuIcon">${icon}</span><span class="menuText">${text}</span>`;
    if(btn.getAttribute('onclick')!==`tab('${id}')`) btn.setAttribute('onclick',`tab('${id}')`);
    btn.dataset.menuId=id;
    btn.title=text;
    if(btn.innerHTML!==wanted) btn.innerHTML=wanted;
  }
  function stabilizeEntregasMenu(){
    ...
    try{
      ...
      let entregaBtns=buttons.filter(b=>btnId(b)==='entrega' || /entregas/i.test(b.textContent||''));
      let btn=entregaBtns[0];
      if(!btn){
        btn=document.createElement('button');
        entregaBtns=[btn];
      }
      entregaBtns.slice(1).forEach(b=>b.remove());
      setButton(btn,'entrega',ENTREGA_ICON,'Entregas');

      const emb=[...nav.querySelectorAll('button')].find(b=>btnId(b)==='embarques');
      if(emb) setButton(emb,'embarques','📦','Embarques');
      const ref=emb || [...nav.querySelectorAll('button')].find(b=>btnId(b)==='transitos');
      if(ref && ref.nextSibling!==btn) nav.insertBefore(btn, ref.nextSibling);
  ...
  function activatePanel(id){
    ...
    const nav=document.querySelector('.sideNav');
    if(nav){
      const active=[...nav.querySelectorAll('button')].find(b=>btnId(b)===id);
    ...
    if(id==='embarques' && typeof window.renderEmbarquesV244==='function') setTimeout(()=>window.renderEmbarquesV244(),30);
    if(id==='clima'){
      if(typeof window.renderClima==='function') setTimeout(()=>window.renderClima(false),30);
      else if(typeof window.ensureClimaDataAndRender==='function') setTimeout(()=>window.ensureClimaDataAndRender(false),30);
      else if(typeof window.renderWeatherFleets==='function') setTimeout(()=>window.renderWeatherFleets(),30);
    }
    if(id==='entrega' && typeof window.renderEntrega==='function') setTimeout(()=>window.renderEntrega(false),30);
  }
  const previousTab=window.tab;
  window.__eltaOriginalTabBeforeEntrega133 = previousTab;
  window.tab=function(id){
    stabilizeEntregasMenu();
    if(id==='entrega'){
      activatePanel('entrega');
      setTimeout(stabilizeEntregasMenu,0);
      return false;
    }
    const r=(typeof previousTab==='function') ? previousTab.apply(this,arguments) : activatePanel(id);
    setTimeout(stabilizeEntregasMenu,0);
    return r;
  };
  function boot(){
    ...
    setVersion133();
    const nav=document.querySelector('.sideNav');
    if(nav && !nav.__entregaMenuObserver133){
      let timer=null;
      const mo=new MutationObserver(()=>{ clearTimeout(timer); timer=setTimeout(stabilizeEntregasMenu,20); });
      mo.observe(nav,{childList:true,subtree:true,attributes:true,attributeFilter:['onclick','class','data-menu-id']});
      nav.__entregaMenuObserver133=mo;
    }
  }
  document.addEventListener('DOMContentLoaded',boot);
