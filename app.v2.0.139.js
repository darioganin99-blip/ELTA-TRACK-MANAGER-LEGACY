...
/* ===== V2.0.139 - SOLO Entregas: evita refresco/flicker al tocar el menu ===== */
(function(){
  const VERSION='2.0.139';
  let entregaBusy=false;
  let entregaRendered=false;
  let entregaMenuSnapshot=null;
  ...
  function entregaButton(){
    return document.querySelector('.sideNav button[data-menu-id="entrega"]') ||
      Array.from(document.querySelectorAll('.sideNav button')).find(b=>(b.getAttribute('onclick')||'').includes("entrega"));
  }

  function preserveEntregaMenu(){
    const btn=entregaButton();
    if(!btn) return;
    btn.setAttribute('data-menu-id','entrega');
    if(!entregaMenuSnapshot){
      entregaMenuSnapshot=btn.innerHTML;
    }else if(btn.innerHTML!==entregaMenuSnapshot){
      btn.innerHTML=entregaMenuSnapshot;
    }
  }

  function activateEntregaOnly(){
  ...
  window.tab=function(id){
    if(id==='entrega'){
      ...
      if(entregaBusy) return false;
      entregaBusy=true;
      const finish=()=>{activateEntregaOnly(); entregaBusy=false; entregaRendered=true;};
      if(!sec && typeof window.renderEntrega==='function'){
        Promise.resolve(window.renderEntrega(false)).then(()=>setTimeout(finish,0)).catch(()=>setTimeout(finish,0));
      }else if(!entregaRendered && typeof window.renderEntrega==='function'){
        Promise.resolve(window.renderEntrega(false)).then(()=>setTimeout(finish,0)).catch(()=>setTimeout(finish,0));
      }else{
        finish();
      }
    ...
    return typeof originalTab==='function' ? originalTab.apply(this,arguments) : undefined;
  };

  document.addEventListener('DOMContentLoaded',()=>{
    setVersionText();
    setTimeout(()=>{preserveEntregaMenu(); setVersionText();},0);
    setTimeout(()=>{preserveEntregaMenu(); setVersionText();},250);
    const nav=document.querySelector('.sideNav');
    if(nav && window.MutationObserver){
      const obs=new MutationObserver(()=>preserveEntregaMenu());
      obs.observe(nav,{childList:true,subtree:true,characterData:true});
    }
  });
  window.addEventListener('load',()=>{preserveEntregaMenu(); setVersionText();});
})();
