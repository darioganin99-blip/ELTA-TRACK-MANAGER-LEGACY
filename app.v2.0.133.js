...
/* ===== V2.0.133 - FIX SEGURO: menu Entregas estable sin romper inicio ===== */
(function(){
  ...
  function setVersionText(){
    ...
    }catch(e){console.warn('version text fix', e);}
  }

  function getNav(){return document.querySelector('.sideNav');}
  function isEntregaButton(btn){
    const onclick=btn.getAttribute('onclick')||'';
    const mid=btn.dataset?.menuId||'';
    const text=(btn.textContent||'').toLowerCase();
    return mid==='entrega' || onclick.includes("tab('entrega')") || onclick.includes('tab("entrega")') || text.includes('entregas');
  }
  function findBtn(id){
    const nav=getNav(); if(!nav) return null;
    return [...nav.querySelectorAll('button')].find(b=>(b.getAttribute('onclick')||'').includes("tab('"+id+"')") || (b.dataset&&b.dataset.menuId===id)) || null;
  }

  function normalizeEntregaMenu(){
    const nav=getNav(); if(!nav) return;
    const buttons=[...nav.querySelectorAll('button')];
    let entregaButtons=buttons.filter(isEntregaButton);
    ...
    const emb=findBtn('embarques');
    if(emb){
      const afterEmb=emb.nextSibling;
      if(afterEmb!==btn) nav.insertBefore(btn, afterEmb);
      const embText=emb.querySelector('.menuText');
      if(embText) embText.textContent='Embarques';
      emb.title='Embarques';
      emb.setAttribute('onclick', "tab('embarques')");
    }
  }
  ...
  function safeTab(id){
    try{
      document.querySelectorAll('.sideNav button').forEach(b=>b.classList.remove('active'));
      document.querySelectorAll('.panel').forEach(p=>p.classList.remove('active'));
      const btn=[...document.querySelectorAll('.sideNav button')].find(b=>(b.getAttribute('onclick')||'').includes("tab('"+id+"')") || (b.dataset&&b.dataset.menuId===id));
      if(btn) btn.classList.add('active');
      const panel=document.getElementById(id);
      if(panel) panel.classList.add('active');

      if(id==='abm' && typeof renderABM==='function') renderABM();
      if(id==='reportes' && typeof renderRep==='function') renderRep();
      if(id==='unidades' && typeof renderUnits==='function') renderUnits();
      if(id==='conductores' && typeof renderDrivers==='function') renderDrivers();
      if(id==='clientes' && typeof renderClients==='function') renderClients();
      if(id==='alertas' && typeof renderAlerts==='function') renderAlerts();
      ...
      if(id==='entrega' && typeof window.renderEntrega==='function') window.renderEntrega(false);

      normalizeEntregaMenu();
      setVersionText();
    }catch(e){
      console.error('safeTab error', e);
    }
  }
  ...
  window.addEventListener('load', bootFix);
  setTimeout(bootFix,50);
  setTimeout(bootFix,500);
  setTimeout(bootFix,1500);

  try{
    const obs=new MutationObserver(()=>{
      if(window.__entregaMenuFixLock) return;
      window.__entregaMenuFixLock=true;
      requestAnimationFrame(()=>{ normalizeEntregaMenu(); setVersionText(); window.__entregaMenuFixLock=false; });
    });
    document.addEventListener('DOMContentLoaded',()=>{const nav=getNav(); if(nav) obs.observe(nav,{childList:true,subtree:true,characterData:true});});
    window.addEventListener('load',()=>{const nav=getNav(); if(nav) obs.observe(nav,{childList:true,subtree:true,characterData:true});});
  }catch(e){console.warn('menu observer fix', e);}
})();
